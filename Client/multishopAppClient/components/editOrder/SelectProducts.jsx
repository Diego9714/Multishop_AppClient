...
import styles from '../../styles/EditSelectProducts.styles';
...
const SelectProducts = ({ isVisible, onClose, selectedOrder, onSave }) => {
  ...
  useEffect(() => {
    applyFilters();
  }, [currentPage, products, displaySearchProduct, searchCategory, priceOrder]);
  ...
  const handleQuantityChange = useCallback((productId, text) => {
    ...
    if (quantity > product.existencia + productOrder.exists) {
  ...
  const handleCancel = () => {
    // Restaurar cantidades de productos al estado inicial
    setProductQuantities(initialProductQuantities);
    
    // Actualizar el contador de productos seleccionados basado en las cantidades iniciales
