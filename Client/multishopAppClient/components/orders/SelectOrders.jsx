...
const SelectOrders = () => {
  ...
  const fetchOrders = async () => {
    try {
      ...
      const cod_cli = decodedToken.cod_cli
      setCodcli(cod_cli)

      const parsedOrders = storedOrdersString ? JSON.parse(storedOrdersString) : []
      const filteredOrders = parsedOrders.filter(order => order.cod_cli === cod_cli)
    ...
    const intervalId = setInterval(async () => {
      try {
        ...
        if (JSON.stringify(parsedOrders) !== JSON.stringify(storedOrders)) {
          const filteredOrders = parsedOrders.filter(order => order.cod_cli === codCli)
          setStoredOrders(filteredOrders)
        }
      } catch (error) {
        console.error('Error fetching orders:', error)
